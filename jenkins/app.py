...
def add_user():
    if request.method == 'POST':
        ...
        con = sql.connect("db_web.db")
        cur = con.cursor()
        cur.execute("insert into users(first_name,last_name,email_p,CONTACT) values (?,?,?,?)", (First_Name ,Last_Name,uname, contact))
        con.commit()
...
def edit_user(uid):
    if request.method == 'POST':
        ...
        con = sql.connect("db_web.db")
        cur = con.cursor()
        cur.execute("update users set email_p=?,CONTACT=? where UID=?", (uname, contact, uid))
        con.commit()
